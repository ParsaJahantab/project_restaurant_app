...
class Breakfast_Menu_Screen extends StatefulWidget {
  final String Number;
  final int res_index;
  Function change1;
  Breakfast_Menu_Screen(this.Number,this.res_index,this.change1);
  @override
  _State createState() => _State();
}

class _State extends State<Breakfast_Menu_Screen> {
  var _fromKey1=GlobalKey<FormState>();
  //bool value;
  String Answer;
  Icon SearchIcon=Icon(Icons.search);
  final TextEditingController filter = new TextEditingController();
  Widget AppbarTitle=Text("breakfast menu");
  change(String text)
  {
    setState(() {
              ...
              onPressed: (){
                setState(() {
                  if (SearchIcon.icon==Icons.search)
                  {
                    ...
                    AppbarTitle=new TextField(
                        onSubmitted: (value){
                          ...
                          {
                            Navigator.of(context).push(MaterialPageRoute(builder: (context) => Search_Screen(widget.Number, widget.res_index,change,value,'breakfast')));
                          }
                        },
                        controller: filter,
                  ...
                  else
                  {
                    ...
                    AppbarTitle=new Text('breakfast menu');
                    setState(() {
                    });
                  }
                });
            ...
            Expanded(
              child: ListView.builder(
                  ...
                  itemBuilder: (BuildContext context,int index){
                    if (restaurant_info.getRestaurants().elementAt(widget.res_index).menu.getMenu().elementAt(index).type=='breakfast')
                    {
                      return InkWell(
